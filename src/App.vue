  ...
  <div class="container">
    ...
    <main>
      ...
      <section class="buttonContainer">
        <button type="button" class="btn btn-primary" @click="handleCheckButtonClick"
          :disabled="answers.size !== questions.length || isCheckingMode">Проверить</button>
        <button type="button" class="btn btn-primary" @click="handleResetButtonClick">Сбросить</button>
      </section>
      <template v-if="isCheckingMode">
        ...
        <section class="feedback">
          <p v-if="feedbackHasSent">Спасибо за Ваш отзыв. Автор получит его, если этому не помешал блокировщик рекламы в
            браузере.
          </p>
          <template v-else>
            <div>
            ...
            <button type="button" class="btn btn-primary" @click="hanldeSendFeedback"
...
const handleCheckButtonClick = async () => {
  ...
  const messageForSend = `[Варыгин] Кто-то прошёл тест c результатом "${resultText.value}".`;
...
const hanldeSendFeedback = () =>
  fetch(urlBase + "[Варыгин] Отзыв: " + feedbackText.value).then(() => {
    feedbackText.value = "";
